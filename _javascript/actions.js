$(document).ready(function(){
	var prodColor = $('.products li.color'),
	    	...
	    	if (deltaY <= -9 && trig == false && products.last().hasClass('active') === false) { // for down scrolling
	    		var cont = product.find('.col-3'),
	    			...
	    			tl.to(prodColor, .3, {opacity: 0, onComplete: function(){
	    				prodColor.attr('data-color', dColor);
	    				tl.to(prodColor, .3, {opacity: 1, onStart: function(){
	    					tl.to(prevColor, .3, {opacity: 0});
	    				}, onComplete: function(){
	    					tl.to(prevColor, .05, {opacity: 1});
	    				}});
	    			}});
	    		...
	    		var cont = prevProd.find('.col-3'),
	    			...
	    			trig = true;
	    			prevColor.attr('data-color', cColor);
	    			tl.to(prevColor, .05, {zIndex: "2", onComplete: function(){
	    				prodColor.attr('data-color', pColor);
	    				tl.to(prevColor, .3, {opacity: 0, onComplete: function(){
	    					tl.to(prevColor, .05, {zIndex: "", opacity: 1});
	    				}});
	    			}});
	    			tl.to(contImg, .15, {opacity: 0});
